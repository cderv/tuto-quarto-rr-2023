...
4.  Comparer le comportement entre
    -   RStudio \> Render depuis la source, ou le *visual editor*
...
````{verbatim}
```{r bill-dims-species, warning=FALSE, fig.width=5, fig.cap="A propos de penguins"}
ggplot(
  penguins,
  aes(
...
Voir `knitr::convert_chunk_header()`
:::
:::
:::
...
1.  Partir de votre `.Rmd`
2.  Utiliser `knitr::convert_chunk_header()` en configurant `output` pour créer un nouveau fichier `.qmd`
3.  Vérifier les configurations des chunks de code
4.  Modifier l'en-tête YAML du fichier `.qmd`
5.  Produire un HTML
:::
...
![](https://github.com/rstudio/bookdown/raw/main/man/figures/logo.png){.absolute fig-alt="Logo bookdown" top="25" right="100" height="120"}
...
| R Markdown                                                                                                                                                                                                 | Quarto                                                                                                                         |
|----------------------------------|--------------------------------------|
| Disponible pour HTML avec **distill** ([Figure avancée](https://rstudio.github.io/distill/figures.html)) ou **tufte** ([Contenu dans la marge](https://rstudio.github.io/tufte/#arbitrary-margin-content)) | [Article Layout](https://quarto.org/docs/authoring/cross-references.html) disponible pour format multiple et plus flexibilité. |
